...
【表記の統一（共同通信社「記者ハンドブック」に準拠。章・ブロックをまたいで同じ判断になるよう、以下を固定の基準とする）】
- 引用・会話・書名・強調はすべて「」を使う。『』は「」の中でさらに引用するときだけ。それ以外の『』はすべて「」に直す。
- 数字は原則アラビア数字にする（施行後3年、10年先送り）。漢数字のまま残すのは、熟語・慣用句（一番、十分、一方、一部、二重）、和語の数え方（一つ、二つ、一人ひとり）、概数（何十年、十数年、数千円、二千年続いた）に限る。迷うものは元のまま残す。
- 単位記号・欧文は半角にする（%、AI）。全角の ％ は使わない。
- 形式名詞はかな書きにする（こと、とき、ところ、もの、わけ、ため、はず）。
- 補助動詞はかな書きにする（〜していく、〜してみる、〜しておく、〜してください）。
- 副詞はかな書きにする（まったく、すでに、ぜひ、もちろん、ほとんど、さらに）。
- 「よい」「ない」「できる」はかな書きにする（良い・無い・出来る と書かない）。
