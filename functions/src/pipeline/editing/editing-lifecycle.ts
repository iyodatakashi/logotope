...
/**
 * 編集ランを開始する。既存の記事（editedChapters ＋ 統合保存 editorial/outputs＝導入/締め/所感）を即時破棄し、
 ...
 * 停止が成立したら、生成に到達しなかった editorial（導入・締め・所感）を生成失敗へ確定して生成待ち表示の固着を防ぐ（Req 4.3）。
 */
...
/**
 * 終端スイープ: 完了（finished）に達しなかった editorial（導入・締め・所感）（生成待ち／生成中／整え中）を完了に確定する（Req 4.3）。
 * 既存内容は保持する（原本があれば編集失敗、無ければ空＝生成失敗）。承認済みペルソナの所感でエントリの無い
 * ／未完了のものにも失敗エントリを materialize する。既に finished の要素は変更しない（冪等・再入安全）。
 */
export const finalizePendingEditorials = async (topicId: string): Promise<void> => {
	...
	for (const kind of ['intro', 'outro'] as const) {
		...
		if (part.status !== 'finished') {
			await narrationWriter(topicId, kind).markEditorialFinished({ draft: part.draft, final: part.final });
		}
	}
	...
	for (let i = 0; i < personas.length; i++) {
		...
		if (!existing || existing.status !== 'finished') {
			...
			await impressionWriter(topicId, persona.id, sortOrder).markEditorialFinished({
