...
async function seedTopic(
	topicId: string,
	personas: SeedPersona[] = defaultPersonas
): Promise<void> {
	...
	for (let i = 0; i < personas.length; i++) {
		const p = personas[i];
		await db()
			.doc(`topics/${topicId}/personas/${p.id}`)
			.set({
				id: p.id,
				topicId,
				...
				sortOrder: i,
				beliefs: [
					{ id: `belief-${p.id}`, version: 0, content: '# 初期信念', createdAt: Timestamp.now() }
				],
...
function makeMockFacilitator(overrides: Partial<Record<string, ReturnType<typeof vi.fn>>> = {}) {
	return {
		generateOpening: vi.fn().mockResolvedValue({
			ok: true,
			value: { content: '討論を始めます。', firstPersonaId: 'p1' }
		}),
		evaluateIntervention: vi
			.fn()
		...
		generateChapterIntroduction: vi.fn().mockResolvedValue({
			ok: true,
			value: { content: '次の章へ移ります。', firstPersonaId: 'p1' }
		}),
		...overrides
	} as unknown as FacilitatorAgentService;
}

type AssessImpl = (personaId: string) => {
	score: number;
...
function makeMockPersonaAgent(
	...
	return {
		...
		generatePostDebateComment: vi.fn().mockImplementation(async (persona: { id: string }) => ({
			ok: true,
			value: { personaId: persona.id, content: '討論後のコメントです。' }
		})),
		...overrides
...
describe('executeChapterTask 統合テスト（Firestore エミュレータ）', () => {
	it('意図キューが章をまたいで復元され、キュー発言として消費される（消失バグの回帰テスト）', async () => {
		...
		const assessImplRef = {
			current: ((personaId: string) => {
				if (personaId === 'p2')
					return { score: 5, mode: 'opinion' as const, intentSummary: '持ち越したい意見' };
		...
		const chapter0 = new DebateOrchestratorService(
			makeMockFacilitator(),
			personaAgent,
			shortOptions
		);
		...
		const chapter1 = new DebateOrchestratorService(
			makeMockFacilitator(),
			makeMockPersonaAgent(assessImplRef),
			shortOptions
		);
		...
		for (const turn of turns) {
			expect(
				turn.chapterIndex,
				`turnIndex=${turn.turnIndex} (${turn.speakerType})`
	...
	it('タスク失敗後のリトライで保存済みターン末尾から再開し、重複ターンを生成しない', async () => {
		...
		const failingAgent = makeMockPersonaAgent(assessImplRef, {
			generateTurn: vi
				...
				.mockResolvedValue({
					ok: false,
					error: { code: 'AI_API_ERROR', message: 'generation failed', retryable: true }
				})
		});
		const firstFacilitator = makeMockFacilitator();
		const firstAttempt = new DebateOrchestratorService(
			firstFacilitator,
			failingAgent,
			shortOptions
		);
		...
		const retryAttempt = new DebateOrchestratorService(
			retryFacilitator,
			makeMockPersonaAgent(assessImplRef),
			shortOptions
		);
