...
export const initAgendaItems = (chapter: Chapter): AgendaItemState[] =>
	(chapter.agenda ?? []).map((point) => ({
		point,
 ...
 */
const getActiveAgendaItemState = (state: DebateState): AgendaItemState | undefined => {
	const introduced = state.agenda.filter(
		(agendaItem) => agendaItem.status === 'introduced'
	);
	if (introduced.length === 0) return undefined;
	return introduced.reduce((latest, agendaItem) =>
		(agendaItem.introducedOrder ?? 0) > (latest.introducedOrder ?? 0)
			? agendaItem
			: latest
	);
};
 ...
 */
export const getActiveAgendaItem = (state: DebateState): string | undefined =>
	getActiveAgendaItemState(state)?.point;
...
export const markIntroduced = (
	...
	const target =
		untouched[index] !== undefined
			? state.agenda.find(
					(agendaItem) => agendaItem.point === untouched[index].point
				)
			: undefined;
	if (!target) return;
	// 直近に提示された論点を一意に追えるよう、提示のたびに単調増加の順序を採番する
	const maxOrder = state.agenda.reduce(
		(max, agendaItem) => Math.max(max, agendaItem.introducedOrder ?? 0),
		0
	);
...
export const markAddressed = (state: DebateState, point: string): void => {
	const target = state.agenda.find((agendaItem) => agendaItem.point === point);
	if (target) target.status = 'addressed';
};

/** 章ドキュメントに論点ステータス（point/status）を書き込む。論点を持たない章では何もしない */
export const saveAgendaItemStatuses = async (
	topicId: string,
	chapterId: string,
	state: DebateState
): Promise<void> => {
	...
	await db()
		...
		.update({
			agendaItemStatuses: state.agenda.map((agendaItem) => ({
				point: agendaItem.point,
...
export const deleteAgendaItemStatuses = async (
	topicId: string,
	chapterId: string
): Promise<void> => {
