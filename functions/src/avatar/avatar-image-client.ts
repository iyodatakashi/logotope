...
import { llmTask, withUsageRecording } from '../llm/usage-recorder.js';

/** リトライ回数（初回＋再試行の合計）。一時失敗を数回吸収し、使い切りで失敗とする。 */
export const MAX_ATTEMPTS = 3;

export const generateImage = llmTask(
	'avatar-image',
	async (prompt: string, seed: Uint8Array): Promise<Uint8Array> => {
		...
		for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
			try {
				const result = await generateText({
					model: withUsageRecording(google(AVATAR_IMAGE_MODEL)),
