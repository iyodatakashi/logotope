      ...
      properties: {
        personas: {
          ...
          items: {
            ...
            additionalProperties: false as const,
            properties: {
              stakeholderRole: { type: 'string' as const, description: 'どのステークホルダー（立場の総称）に対応するか。立場リストの総称をそのまま記入する（例: F1チーム関係者）' },
              specificRole: { type: 'string' as const, description: 'このテーマにおけるその人物の具体的な立場・肩書き。stakeholderRole が「F1チーム関係者」のような総称の場合は、オーナー／レースエンジニア／メカニックなど具体的な役職に必ず特定する。「F1の熱心なファン」のように総称が既に具体的ならそれを反映する。occupation（実生活上の職業）とは別物で、ファンなど職業外で関わる人物では両者は異なる' },
