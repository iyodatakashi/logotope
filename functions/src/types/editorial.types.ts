...
// editorial（導入・締め・所感）の進捗ステータス。生成待ち／原本生成中／整え中／処理完了を表す。
...
// 導入・締めの editorial。進捗ステータスと、原本 draft・編集後 final を持つ（未生成/失敗は null）。
...
// 所感の editorial（承認ペルソナごと）。sortOrder で表示順を保ち、進捗ステータスと原本 draft・編集後 final を持つ。
...
// 導入・締め・所感をまとめた統合ドキュメント editorial/outputs。所感は personaId をキーにしたマップにし、
