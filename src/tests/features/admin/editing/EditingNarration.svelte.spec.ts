...
describe('EditingNarration.svelte（状態駆動表示）', () => {
	describe('進行中はスケルトン（段階ラベル）・再生成なし', () => {
		it('pending: 段階ラベルも本文も再生成も出さない', async () => {
			render(EditingNarration, makeProps({ status: 'pending', draft: null, final: null }));
		...
		it('generating: 「生成中」ラベルを出し再生成は出さない', async () => {
			render(EditingNarration, makeProps({ status: 'generating', draft: null, final: null }));
			await expect.element(page.getByText('生成中')).toBeInTheDocument();
			expect(regenerate().elements()).toHaveLength(0);
		});

		it('editing: 「編集中」ラベルを出し再生成は出さない', async () => {
			render(EditingNarration, makeProps({ status: 'editing', draft: '原本', final: null }));
	...
	describe('完了は内容から成否を表示・再生成あり', () => {
		it('編集済み（final あり）: 編集後本文を表示し状態ラベルは出さない', async () => {
			render(EditingNarration, makeProps({ status: 'finished', draft: '原本', final: '編集後本文' }));
		...
		it('編集失敗（原本のみ）: 原本本文＋「編集失敗」＋再生成', async () => {
			render(EditingNarration, makeProps({ status: 'finished', draft: '原本本文', final: null }));
		...
		it('生成失敗（空）: 本文を出さず「生成失敗」＋再生成', async () => {
			render(EditingNarration, makeProps({ status: 'finished', draft: null, final: null }));
