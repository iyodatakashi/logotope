...
> **R6（ペルソナの過剰指名抑制）は本イテレーションでは実施しない（意図的延期）。**
> 理由: 指名抑制を同時に入れると指名連鎖そのものが減り、コアの仕組み（指名チェーン中の drift 出尽くし判断＝R1〜R5）が効いたのか、単に入力（指名頻度）が減っただけなのかを切り分けられなくなる。まずコアの介入機構の有効性を検証し、その結果を見て R6 の要否・度合いを別途判断する。
...
- R6（過剰指名の抑制）: **本イテレーションでは延期**（冒頭の理由を参照）。コア機構の検証後に別途タスク化を判断。
